...
# Ported from the standalone Bull Market Advanced Stock Market Trading Robot v8.
...
#   1. Latest 14-period daily RSI >= BULL_RSI_BUY_THRESHOLD (default 65)  -- momentum
#   2. current_price <= last_price * BULL_DIP_FACTOR (default 0.998)       -- dip
# The path uses this bot's already-cached RSI/price data, not the pasted
# strategy's separate yfinance calls, so it costs no extra network requests.
USE_BULL_MARKET_STRATEGY = True
BULL_RSI_BUY_THRESHOLD = 65.0
BULL_DIP_FACTOR = 0.998                    # buy when current <= last * this
...
def _bull_market_scan_candidates(symbols_to_buy_list, regime, total_symbols_for_alloc):
    """
    Bull Market Advanced Stock Market Trading Robot v8 buy path.

    Runs only when regime == REGIME_BULL and USE_BULL_MARKET_STRATEGY is on.
    For each symbol, fires if EITHER:
      - Latest daily RSI(14) >= BULL_RSI_BUY_THRESHOLD (momentum)
      - current_price <= last_close * BULL_DIP_FACTOR (small dip)

    Returns a list of candidate dicts in the same shape buy_stocks Phase 2
    expects, tagged with 'bull_strategy': True so Phase 2 sizes with the
    per-symbol cash cap, places the mandatory 1% trailing stop, and persists
    the position with strategy_tag='bull'. Symbols are NOT claim()'d here --
    the main buy_stocks loop is the single owner of position_book claims and
    will de-dupe by api_symbol before running Phase 2.
    """
    ...
    # capped at the per-symbol max. This mirrors the pasted strategy's
    # allocate_cash_equally() but uses the total scanner universe as the
    ...
    print(f"[bull] regime=BULL; per-symbol allocation ${bull_alloc:.2f} "
    ...
    for symbol in list(symbols_to_buy_list):
        ...
        try:
            ...
            # Both trigger inputs come from data this bot already caches, so
            # this scan costs zero extra yfinance requests on a warm cache.
            ...
            rsi_trigger = daily_rsi is not None and daily_rsi >= BULL_RSI_BUY_THRESHOLD
            dip_trigger = current_price <= last_close * BULL_DIP_FACTOR
            if not (rsi_trigger or dip_trigger):
                continue

            reasons = []
            if rsi_trigger:
                reasons.append(f"bull_rsi>={BULL_RSI_BUY_THRESHOLD:.0f} ({daily_rsi:.1f})")
            if dip_trigger:
                reasons.append(f"bull_dip<={BULL_DIP_FACTOR:.3f}x_last "
                               f"(${current_price:.2f}<=${last_close*BULL_DIP_FACTOR:.2f})")
            ...
            print(f"[bull] candidate {yf_symbol}: {', '.join(reasons)} "
                  f"| price ${current_price:.2f} | RSI {daily_rsi if daily_rsi is not None else 'n/a'} "
                  f"| alloc ${bull_alloc:.2f}")
        except Exception as e:
            logging.warning(f"[bull] scan error for {yf_symbol}: {e}")
            continue
