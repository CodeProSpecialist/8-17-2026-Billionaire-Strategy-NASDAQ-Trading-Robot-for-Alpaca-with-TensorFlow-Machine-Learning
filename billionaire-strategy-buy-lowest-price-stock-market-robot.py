...
def _bull_batch_sample_prices(symbols):
    """
    Fetch the latest 1-minute intraday close for ALL bull-monitored symbols
    in ONE yfinance HTTP call, using yf_download_batch (which consumes just
    one slot from the bot's global yf_gate rate limiter).

    Returns {symbol: last_close_float}. Symbols that came back empty or
    invalid are omitted -- caller treats them as a missed sample for that
    round only.

    This is why we don't need a per-worker rate limiter for the bull path:
    at 15 symbols x 18 samples over 3 minutes, per-symbol fetches would be
    270 requests. Batching drops that to 18 total (one per sample round),
    a 15x reduction, so the shared yf_gate handles it comfortably no matter
    how many symbols are in play.
    """
    if not symbols:
        return {}
    try:
        batch = yf_download_batch(symbols, period="1d", interval="1m")
    except Exception as e:
        logging.warning(f"[bull] batch price fetch failed: {e}")
        return {}
...
def print_portfolio_gain_summary(force=False):
    ...
    try:
        # 1M / 1D gives ~22 trading-day closes -- more than enough for a
        # 14-trading-day lookback. Alpaca returns lists in parallel: timestamp
        # (unix), equity (float), profit_loss, etc.
    ...
    def window_result(n_sessions, label):
        ...
        if len(ordered) <= n_sessions:
            return f"  {label:<10}: insufficient history ({len(ordered)-1} session(s) available)"
        ...
        return (f"  {label:<10}: {arrow} {pct:+.2f}%  "
                f"(${prior_equity:,.2f} on {prior_date} -> ${latest_equity:,.2f} on {latest_date}, "
    ...
    lines = [
        ...
        f"  Fetched at: {fetched_at_et}  (refetches at most 1/hour)",
        "============================================================================",
        "",
    ]
    text = "\n".join(lines)
