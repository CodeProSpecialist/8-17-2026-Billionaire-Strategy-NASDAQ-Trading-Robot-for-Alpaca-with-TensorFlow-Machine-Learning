...
# specialization.
#
# ARCHITECTURE (V2 two-stream sequence-model TFBrain shape):
#     Input: (batch, ML_BRAIN_SEQ_LEN=20, ML_BRAIN_FEATURES=32)
#            = 20 daily snapshots, each = 10 V1 stock features + 2 relative-
#              strength features (stock vs NASDAQ) + 20 NASDAQ features.
#          |
#     TimeDistributed(Dense(13, tanh))  <-- FOUNDATION 1 (frozen)
#          |  8 original stock-setup detectors + 4 NASDAQ regime detectors
#          |  + 1 relative-strength detector
#     TimeDistributed(Dense(10, tanh))   <-- FOUNDATION 2 (frozen)
#          |  6 original setup summaries + 2 NASDAQ-conditioned summaries
#          |  + 2 interaction summaries (stock-setup x environment)
#          |
...
#
# V2 CHANGES (Steps 1, 3, 6 of the world-class design, plus training-quality
# fixes):
#   - NASDAQ market-state stream alongside per-stock state, so the model can
#     learn "this dip-buy setup only works when NASDAQ is trending".
#   - Stock vs NASDAQ relative-strength features -- the classical outperformance
#     signal, one of the highest-alpha inputs a NASDAQ book can have.
#   - Interaction summary units in foundation_2 that hand the LSTM
#     stock-setup x environment combinations pre-composed instead of asking
#     it to learn multiplicative priors from a limited training set.
#   - MACD normalized to price so cross-symbol training isn't burning capacity
#     on per-magnitude normalization.
#   - Chronological train/val split (was: random shuffle), so val_accuracy
#     stops being optimistically biased by same-date correlated leakage.
#   - EarlyStopping on val_loss with restored best weights.
#   - Data-driven inference threshold (median of val predictions) instead of
#     the hard-coded 0.5, so focal-loss miscalibration stops silently biasing
#     the score adjustment sign.
#   The stock feature block indices 0-9 are byte-for-byte identical to V1, and
#   the frozen foundation's original 8 stock-detector weights are preserved
#   verbatim (with zeros in the new columns), so V1 behavior is a strict
#   subset of V2 behavior on any purely-stock input.
#   The 60-live-trade gate, ±1.5 score cap, exit-simulated labels, focal loss,
#   walk-forward evaluation, and daily 17:00 ET schedule are all unchanged.
#
# TRAINING SIGNAL:
...
# V2 upgrade: per-day feature count now = 12 stock features + 20 NASDAQ market-state
# features. The first 10 stock features (indices 0-9) are unchanged from V1 so
# the frozen foundation's original stock detectors keep firing on the exact same
# inputs. Indices 10-11 are V2 additions: stock relative-strength vs NASDAQ
# (stock_ret - nas_ret, 1d and 20d). Indices 12-31 are the NASDAQ market-state
# block described in the V2 design doc.
ML_BRAIN_STOCK_FEATURES = 12       # was 10: +2 stock-vs-NASDAQ relative-strength slots
ML_BRAIN_NASDAQ_FEATURES = 20
ML_BRAIN_FEATURES = ML_BRAIN_STOCK_FEATURES + ML_BRAIN_NASDAQ_FEATURES   # 32
# Symbol used as the NASDAQ market-state proxy. QQQ (NASDAQ-100 ETF) has cleaner
# yfinance intraday/daily data than ^IXIC and moves ~1:1 with the composite for
# regime purposes. Kept as a single constant so it's easy to swap for ^NDX later.
ML_NASDAQ_PROXY_SYMBOL = 'QQQ'
ML_NASDAQ_LOOKBACK_DAYS = 260   # ~1yr of daily bars: enough for SMA200 + slope
...
def _ml_build_model(tf):
    ...
    Architecture (V2 -- 32 features per day, 12 stock + 20 NASDAQ):
        Input: (batch, 20, 32) -- rolling window of daily feature snapshots
              |
        TimeDistributed(Dense(13, tanh)) <-- FOUNDATION 1: per-timestep prior
              |     [FROZEN -- hand-specified weights, trainable=False]
              |     8 stock detectors (V1, preserved) + 4 NASDAQ detectors (V2)
              |     + 1 relative-strength detector (V2)
        TimeDistributed(Dense(10, tanh)) <-- FOUNDATION 2: refined per-timestep prior
              |     [FROZEN -- hand-specified weights, trainable=False]
              |     6 stock summaries (V1, preserved) + 2 NASDAQ summaries
              |     + 2 interaction summaries (stock-setup x environment)
              |
    ...
    foundation_1 = layers.TimeDistributed(
        layers.Dense(13, activation='tanh', name='foundation_1_inner'),
        name='foundation_1',
    )
    foundation_1.trainable = False

    foundation_2 = layers.TimeDistributed(
        layers.Dense(10, activation='tanh', name='foundation_2_inner'),
        name='foundation_2',
    )
...
def _ml_set_foundation_weights(model):
    ...
    """
    import numpy as np
    # V2: 32 features (10 V1 stock indices 0-9, 2 rel-strength indices 10-11,
    # 20 NASDAQ indices 12-31), 13-unit foundation_1 (8 original stock detectors
    # + 4 NASDAQ detectors + 1 relative-strength detector), 10-unit foundation_2
    # (6 original summaries + 2 NASDAQ summaries + 2 interaction summaries).
    # The V1 stock-detector weights are preserved BYTE-FOR-BYTE by writing them
    # into columns 0-7 of W1 (rows 0-9 are the same stock features they were in V1);
    # all rows 10-31 (rel-strength + NASDAQ) stay ZERO for units 0-7, so V1
    # detectors ignore new inputs entirely -- identical firing to V1 on stock data.
    n_features = ML_BRAIN_FEATURES   # 32 (was 10 in V1)
    n_hidden_1 = 13                  # was 8 in V1: +4 NASDAQ +1 rel-strength
    n_hidden_2 = 10                  # was 6 in V1: +2 NASDAQ +2 interaction

    # Feature index reminder (V2):
    #   Stock block (V1 preserved):
    #     0 RSI/100, 1 MACD/price, 2 MACD_sig/price, 3 MACD>signal, 4 ATR%,
    #     5 daily return, 6 volume vs SMA, 7 close vs SMA20,
    #     8 close vs SMA50, 9 SMA20>SMA50
    #   Stock block (V2 additions):
    #     10 rel-strength 1d (stock_ret - nas_ret),
    #     11 rel-strength 20d (stock_ret20 - nas_ret20)
    #   NASDAQ block (shifted +2 from prior V2 draft, indices 12-31):
    #     12 NAS_RSI/100, 13 NAS_MACD/price, 14 NAS_MACD_sig/price,
    #     15 NAS_MACD>signal, 16 NAS_ATR%, 17 NAS_daily return,
    #     18 NAS_5d return, 19 NAS_20d return, 20 NAS_60d return,
    #     21 NAS_dist SMA20, 22 NAS_dist SMA50, 23 NAS_dist SMA100,
    #     24 NAS_dist SMA200, 25 NAS_SMA20 slope, 26 NAS_SMA50 slope,
    #     27 NAS_SMA20>SMA50, 28 NAS_realized vol 20d, 29 NAS_vol percentile,
    #     30 NAS_volume vs SMA, 31 NAS_ROC(10)

    # ---- Foundation 1 weights: (n_features, n_hidden_1) = (32, 13) ----
    ...
    # ---- V2 NASDAQ DETECTORS (units 8-11) ----
    # These fire on the NASDAQ market-state block (feature indices 12-31). They
    # let the model condition every per-stock decision on "what kind of NASDAQ
    # environment are we in?" -- the environment-vs-instrument split from the
    # V2 design.

    # unit 8: NASDAQ_BULL_TREND -- SMA20>SMA50 AND price above SMA50 AND positive SMA20 slope
    W1[27, 8] = 2.0   # NAS_SMA20>SMA50 flag
    W1[22, 8] = 10.0  # NAS distance from SMA50 (typically -0.10 to +0.10)
    W1[25, 8] = 40.0  # NAS SMA20 slope (typically -0.01 to +0.01 per day) -- big weight, small values
    b1[8] = -1.0

    # unit 9: NASDAQ_BEAR_TREND -- SMA20<SMA50 AND below SMA50 AND negative momentum
    W1[27, 9] = -2.0  # NAS_SMA20>SMA50 flag being FALSE (0) -> no contribution; TRUE -> penalize
    W1[22, 9] = -10.0 # negative when NAS is below SMA50
    W1[19, 9] = -8.0  # negative NAS 20d return
    b1[9] = -0.5      # only fires with genuinely bearish tape

    # unit 10: NASDAQ_HIGH_VOL -- elevated ATR% and realized volatility
    W1[16, 10] = 50.0 # NAS_ATR% (typical ~0.008 for QQQ -> +0.4)
    W1[28, 10] = 30.0 # NAS realized 20d vol (annualized decimal; ~0.15 typical -> +4.5, capped by tanh)
    b1[10] = -1.2

    # unit 11: NASDAQ_MOMENTUM -- multi-horizon return blend + RSI push
    # Recalibrated V2: original weights only pushed tanh to +0.16 on bullish
    # tape (weak). Bumped daily return weight and RSI weight so a clearly
    # positive tape (RSI 0.65, +1% daily, +4% 20d) pushes tanh comfortably
    # above +0.5 without saturating on typical values.
    W1[17, 11] = 40.0   # daily return (typical +/- 0.01 -> +/- 0.4)
    W1[18, 11] = 12.0   # 5d return
    W1[19, 11] = 6.0    # 20d return
    W1[12, 11] = 4.0    # NAS RSI/100 (0.5=neutral, 0.7=strong -> +2.8, 0.3=weak -> +1.2)
    b1[11] = -2.4       # centered so tanh output near 0 in neutral tape (RSI 0.5, zero returns)

    # ---- V2 RELATIVE-STRENGTH DETECTOR (unit 12) ----
    # unit 12: STOCK_OUTPERFORMS_NASDAQ -- stock leading the tape.
    #   Reads only stock indices 10-11 (rel-strength 1d and 20d). This is the
    #   classical relative-strength signal: buying strength that's beating the
    #   market is meaningfully different (and historically higher-alpha) than
    #   buying a stock that's just moving with the tape. Fires positive when
    #   the stock is outpacing NASDAQ, negative when lagging.
    W1[10, 12] = 30.0   # rel-strength 1d (typical +/- 0.01 -> +/- 0.3)
    W1[11, 12] = 15.0   # rel-strength 20d (typical +/- 0.05 -> +/- 0.75)
    b1[12] = 0.0        # symmetric around zero: neutral rel-strength -> tanh(0)=0
    ...
    # ---- Foundation 2 weights: (n_hidden_1, n_hidden_2) = (13, 10) ----
    # Input is the tanh-activated output of foundation_1, in ~[-1, +1] per unit.
    # V1 units 0-5 are preserved verbatim -- they only read stock-detector inputs
    # (rows 0-7), so their behavior on the stock block is byte-for-byte the V1
    # behavior. Units 6-7 are NASDAQ-conditioned summaries. Units 8-9 are V2
    # interaction summaries (stock-setup x environment) -- the multiplicative
    # priors the LSTM would otherwise have to learn from scratch.
    ...
    #   8=NAS_BULL_TREND, 9=NAS_BEAR_TREND, 10=NAS_HIGH_VOL, 11=NAS_MOMENTUM,
    #   12=REL_STRENGTH
    ...
    # ---- V2 NASDAQ-CONDITIONED SUMMARIES (units 6-7) ----

    # unit 6: "green-light environment" -- NASDAQ trending bull AND not high vol
    #   Combines the two new NASDAQ regime detectors into a single "the market
    #   is helping us" signal. Head learns to lean harder on stock setups when
    #   this fires positive.
    W2[8, 6] = 1.2    # NAS_BULL_TREND positive
    W2[9, 6] = -1.0   # NAS_BEAR_TREND negates
    W2[10, 6] = -0.6  # NAS_HIGH_VOL discounts (bull chop still risky)
    W2[11, 6] = 0.5   # NAS_MOMENTUM confirms
    b2[6] = -0.3

    # unit 7: "red-light environment" -- bear tape OR panic volatility
    #   Symmetric caution signal. Fires positive when NASDAQ is a bad place
    #   to be dip-buying regardless of how clean an individual stock setup looks.
    W2[9, 7] = 1.3    # NAS_BEAR_TREND directly
    W2[10, 7] = 1.0   # NAS_HIGH_VOL amplifies
    W2[11, 7] = -0.8  # positive NAS momentum suppresses
    W2[8, 7] = -0.8   # bull-trend suppresses
    b2[7] = -0.4

    # ---- V2 INTERACTION SUMMARIES (units 8-9) ----
    # These are the multiplicative-style priors that make regime awareness
    # actually change decisions. Without them, foundation_2 hands the LSTM
    # a set of siblings (stock setup, environment) and asks it to learn the
    # interaction from scratch across a limited training set. With them, the
    # highest-alpha combinations are handed to the LSTM pre-composed.

    # unit 8: "confluent dip-buy" -- stock setup AND environment agree.
    #   High only when DIP_BUY fires positive AND green-light-ish signals
    #   (NAS_BULL_TREND or NAS_MOMENTUM) are also positive. Because linear
    #   combos in tanh can't do a true AND, we use large-ish weights on both
    #   inputs plus a strongly negative bias so a single input alone barely
    #   activates: both must be positive together to push tanh well above 0.
    W2[3, 8] = 1.2    # DIP_BUY_COMPOSITE (stock)
    W2[8, 8] = 1.0    # NAS_BULL_TREND (environment)
    W2[11, 8] = 0.6   # NAS_MOMENTUM adds
    W2[7, 8] = -0.8   # BEARISH stock detector suppresses
    W2[9, 8] = -1.2   # NAS_BEAR_TREND kills it entirely
    b2[8] = -1.4      # bias low so requires simultaneous positive signals

    # unit 9: "leader in an uptrend" -- rel-strength AND healthy uptrend.
    #   The single highest-alpha combination in a NASDAQ-focused book:
    #   buying a stock that's outpacing the tape AND the stock itself is in
    #   its own confirmed uptrend. If either half is missing this reads
    #   near zero; both together push tanh strongly positive.
    W2[12, 9] = 1.4   # REL_STRENGTH positive
    W2[2, 9] = 0.9    # UPTREND
    W2[6, 9] = 0.7    # SHORT_STRENGTH
    W2[9, 9] = -1.0   # NAS_BEAR_TREND still overrides (leader in a crash is still risky)
    b2[9] = -1.2
...
def _ml_build_feature_row(rsi_val, macd_val, macd_sig_val, atr_val, close_val,
    ...
    close_safe = max(close, 0.01)
    # V2 improvement: MACD raw values scale with absolute price ($0.05 on a
    # $10 stock, $2 on a $200 stock), so mixing symbols across the universe
    # forces the LSTM to spend capacity learning per-symbol normalization.
    # Normalizing to price puts every symbol on the same dimensionless scale
    # and lets the pattern (not the magnitude) drive learning.
    macd_f = _fin(macd_val, 0.0) / close_safe
    macd_sig_f = _fin(macd_sig_val, 0.0) / close_safe
    return [
        _fin(rsi_val, 50.0) / 100.0,                           # 0: RSI normalized [0,1]
        macd_f,                                                 # 1: MACD/price (was: raw)
        macd_sig_f,                                             # 2: MACD signal/price (was: raw)
        1.0 if _fin(macd_val) > _fin(macd_sig_val) else 0.0,   # 3: MACD-above-signal indicator
        _fin(atr_val, 0.0) / close_safe,                        # 4: ATR%
...
# =================================================================================
# ---------------- V2 NASDAQ MARKET-STATE FEATURES --------------------------------
# =================================================================================
# The NASDAQ block is the "environment" input Step 1 of the V2 design calls for.
# Same yfinance rate-limited fetcher the rest of the file uses. The feature
# vector is time-aligned to the per-stock daily bars via the shared trading
# calendar, so a stock-day-N row is concatenated with the SAME calendar day's
# NASDAQ state -- no look-ahead, no misalignment.
_ml_nasdaq_cache = {'df': None, 'features_by_date': None, 'built_at': None}
_ml_nasdaq_cache_lock = threading.Lock()
_ML_NASDAQ_CACHE_TTL_SECS = 60 * 60 * 4   # refresh every 4 hours during trading day


def _ml_build_nasdaq_feature_row(idx, close, high, low, volume, rsi, macd,
                                  macd_sig, atr, sma20, sma50, sma100, sma200,
                                  daily_ret, ret_5d, ret_20d, ret_60d,
                                  sma20_slope, sma50_slope, realized_vol_20d,
                                  vol_percentile_proxy, vol_vs_sma, roc_10):
    """One day's NASDAQ market-state row (20 features, indices 10-29 of the
    concatenated per-day vector). All quantities are dimensionless or normalized
    to the closing price so they stay in a similar dynamic range regardless of
    where QQQ happens to be trading. Any non-finite input falls back to a
    neutral value so a single missing bar can't NaN-poison a whole sequence."""
    def _fin(x, default=0.0):
        try:
            v = float(x)
            return v if np.isfinite(v) else default
    ...
    c = _fin(close[idx], 1.0)
    c_safe = max(c, 0.01)
    # V2: NASDAQ MACD also normalized to price for consistency with the
    # stock block and so the value stays in a small dimensionless range
    # regardless of what QQQ is trading at.
    nas_macd = _fin(macd[idx], 0.0) / c_safe
    nas_macd_sig = _fin(macd_sig[idx], 0.0) / c_safe
    return [
        _fin(rsi[idx], 50.0) / 100.0,                                      # 10 NAS RSI/100
        nas_macd,                                                           # 11 NAS MACD/price
        nas_macd_sig,                                                       # 12 NAS MACD signal/price
        1.0 if _fin(macd[idx]) > _fin(macd_sig[idx]) else 0.0,             # 13 NAS MACD > signal
        _fin(atr[idx], 0.0) / c_safe,                                       # 14 NAS ATR%
        _fin(daily_ret[idx], 0.0),                                          # 15 NAS daily return
        _fin(ret_5d[idx], 0.0),                                             # 16 NAS 5d return
        _fin(ret_20d[idx], 0.0),                                            # 17 NAS 20d return
        _fin(ret_60d[idx], 0.0),                                            # 18 NAS 60d return
        (c / max(_fin(sma20[idx], c), 0.01)) - 1.0,                        # 19 NAS dist from SMA20
        (c / max(_fin(sma50[idx], c), 0.01)) - 1.0,                        # 20 NAS dist from SMA50
        (c / max(_fin(sma100[idx], c), 0.01)) - 1.0,                       # 21 NAS dist from SMA100
        (c / max(_fin(sma200[idx], c), 0.01)) - 1.0,                       # 22 NAS dist from SMA200
        _fin(sma20_slope[idx], 0.0),                                        # 23 NAS SMA20 slope
        _fin(sma50_slope[idx], 0.0),                                        # 24 NAS SMA50 slope
        1.0 if _fin(sma20[idx], 0) > _fin(sma50[idx], 0) else 0.0,         # 25 NAS SMA20>SMA50
        _fin(realized_vol_20d[idx], 0.0),                                   # 26 NAS realized 20d vol
        _fin(vol_percentile_proxy[idx], 0.0),                               # 27 NAS vol vs 60d percentile
        _fin(vol_vs_sma[idx], 0.0),                                         # 28 NAS volume vs SMA
        _fin(roc_10[idx], 0.0),                                             # 29 NAS ROC(10)
    ]


def _ml_neutral_nasdaq_row():
    """20-length list of neutral values, used when the NASDAQ block is
    unavailable for a given calendar day (very old date pre-QQQ, missing bar,
    or fetch failure). Zeros for all bounded/normalized fields; RSI at 0.5
    (neutral); SMA-flag at 0. This is the "no opinion" prior -- keeps the
    stock detectors dominant when the environment signal isn't reliable."""
    row = [0.0] * ML_BRAIN_NASDAQ_FEATURES
    row[0] = 0.5   # NAS RSI/100 -> neutral 50
    return row


def _ml_compute_nasdaq_features_frame(df):
    """Given a NASDAQ (QQQ) OHLCV daily DataFrame, return a dict
    {date -> 20-length feature list}. Runs all TA-lib calls once and vectorizes
    the slope/return calculations; caller can then look up any date cheaply."""
    ...
    try:
        ...
        vol_sma20 = (talib.SMA(volume, timeperiod=20)
                     if volume.sum() > 0 else np.full(len(close), 1.0))
        roc_10 = talib.ROC(close, timeperiod=10) / 100.0  # decimal
    except Exception as e:
        logging.warning(f"ml_brain V2 NASDAQ feature calc failed: {e}")
        return {}

    n = len(close)
    daily_ret = np.zeros(n)
    daily_ret[1:] = (close[1:] - close[:-1]) / np.maximum(close[:-1], 0.01)

    def _horizon_return(arr, k):
        out = np.zeros(len(arr))
        out[k:] = (arr[k:] - arr[:-k]) / np.maximum(arr[:-k], 0.01)
        return out

    ret_5d = _horizon_return(close, 5)
    ret_20d = _horizon_return(close, 20)
    ret_60d = _horizon_return(close, 60)

    def _slope(arr, window):
        out = np.zeros(len(arr))
        for i in range(window, len(arr)):
            prev = arr[i - window]
            if np.isfinite(prev) and prev > 0 and np.isfinite(arr[i]):
                out[i] = (arr[i] - prev) / (window * prev)  # avg per-day fractional change
        return out

    sma20_slope = _slope(sma20, 5)
    sma50_slope = _slope(sma50, 10)

    # Realized 20d vol: std of daily returns * sqrt(252), annualized decimal
    realized_vol_20d = np.zeros(n)
    for i in range(20, n):
        window = daily_ret[i - 20 + 1: i + 1]
        realized_vol_20d[i] = float(np.std(window)) * (252 ** 0.5)

    # Volatility percentile proxy: current 20d vol vs its own rolling 60d mean
    vol_percentile_proxy = np.zeros(n)
    for i in range(80, n):
        recent = realized_vol_20d[i - 60 + 1: i + 1]
        mean = float(np.mean(recent))
        vol_percentile_proxy[i] = (realized_vol_20d[i] / mean) - 1.0 if mean > 0 else 0.0

    vol_vs_sma = np.zeros(n)
    for i in range(len(vol_sma20)):
        s = float(vol_sma20[i]) if np.isfinite(vol_sma20[i]) else 0.0
        vol_vs_sma[i] = (float(volume[i]) / s) - 1.0 if s > 0 else 0.0

    out = {}
    for i in range(210, n):  # start after SMA200 stabilizes
        try:
            d = df.index[i]
            key = d.date() if hasattr(d, 'date') else d
        except Exception:
            continue
        out[key] = _ml_build_nasdaq_feature_row(
            i, close, high, low, volume, rsi, macd, macd_signal, atr,
            sma20, sma50, sma100, sma200, daily_ret, ret_5d, ret_20d, ret_60d,
            sma20_slope, sma50_slope, realized_vol_20d, vol_percentile_proxy,
            vol_vs_sma, roc_10)
    return out


def _ml_get_nasdaq_features_map(force_refresh=False):
    """Fetch (or return cached) map of {date -> 20-feature NASDAQ row}.
    Thread-safe; refreshed every _ML_NASDAQ_CACHE_TTL_SECS. Used by both
    training (build one big historical map once) and live inference (look up
    today's row cheaply)."""
    with _ml_nasdaq_cache_lock:
        now = time.time()
        cached_map = _ml_nasdaq_cache.get('features_by_date')
        built = _ml_nasdaq_cache.get('built_at') or 0
        if (not force_refresh and cached_map
                and (now - built) < _ML_NASDAQ_CACHE_TTL_SECS):
            return cached_map

        try:
            end_date = datetime.now(eastern).date()
            start_date = end_date - timedelta(days=int(365 * ML_HIST_LOOKBACK_YEARS) + 400)
            df = yf_history(ML_NASDAQ_PROXY_SYMBOL,
                            start=start_date.isoformat(),
                            end=(end_date + timedelta(days=1)).isoformat(),
                            interval='1d')
        except Exception as e:
            logging.warning(f"ml_brain V2 NASDAQ fetch failed: {e}")
            return cached_map or {}

        features_map = _ml_compute_nasdaq_features_frame(df)
        if features_map:
            _ml_nasdaq_cache['df'] = df
            _ml_nasdaq_cache['features_by_date'] = features_map
            _ml_nasdaq_cache['built_at'] = now
            logging.info(f"ml_brain V2: NASDAQ feature map built ({len(features_map)} dates)")
        return features_map or (cached_map or {})


def _ml_lookup_nasdaq_row_for_date(features_map, date_obj):
    """Look up the NASDAQ feature row for a given calendar date, falling back
    to the most-recent prior date (up to 7 calendar days back, to bridge
    weekends and holidays without look-ahead). Returns the neutral row when
    no prior date is available."""
    if not features_map or date_obj is None:
        return _ml_neutral_nasdaq_row()
    if hasattr(date_obj, 'date') and not isinstance(date_obj, date):
        date_obj = date_obj.date()
    for back in range(0, 8):
        probe = date_obj - timedelta(days=back)
        if probe in features_map:
            return features_map[probe]
    return _ml_neutral_nasdaq_row()
...
def _ml_build_examples_for_symbol(symbol, df, n_examples_wanted):
    ...
    # V2: fetch NASDAQ market-state map once per training call so every
    # stock-day row gets time-aligned market context. If the map is empty
    # (e.g. yfinance NASDAQ fetch failed), rows fall back to neutral NASDAQ
    # features, which is equivalent to V1 behavior for those specific days.
    nasdaq_map = _ml_get_nasdaq_features_map()
    has_datetime_index = hasattr(df.index, 'to_pydatetime')

    # Pre-build every day's feature row once, then slice sequences from it.
    # Row layout: [10 V1 stock features | 2 rel-strength | 20 NASDAQ] = 32 total.
    n_days = len(close)
    feature_rows = np.zeros((n_days, ML_BRAIN_FEATURES), dtype=np.float32)
    for i in range(n_days):
        stock_row = _ml_build_feature_row(
            rsi[i], macd[i], macd_signal[i], atr[i], close[i],
            volume[i], vol_sma[i], sma20[i], sma50[i], daily_return[i])
        # Time-align NASDAQ features to this stock bar's calendar date
        if has_datetime_index:
            try:
                bar_date = df.index[i].to_pydatetime().date()
            except Exception:
                bar_date = None
        else:
            bar_date = None
        nasdaq_row = _ml_lookup_nasdaq_row_for_date(nasdaq_map, bar_date)
        # V2 relative-strength (stock indices 10-11): stock_return - nasdaq_return.
        # NASDAQ block index 5 = daily return (=block idx 15 overall); NASDAQ
        # block index 7 = 20d return. When either NASDAQ or stock return is
        # unavailable, rel-strength defaults to 0 (neutral).
        nas_daily = nasdaq_row[5] if len(nasdaq_row) > 5 else 0.0     # NAS daily ret
        nas_ret20 = nasdaq_row[7] if len(nasdaq_row) > 7 else 0.0     # NAS 20d ret
        if i >= 20 and close[i-20] > 0:
            stock_ret20 = (close[i] - close[i-20]) / close[i-20]
        else:
            stock_ret20 = 0.0
        rel_strength_1d = float(daily_return[i]) - float(nas_daily)
        rel_strength_20d = float(stock_ret20) - float(nas_ret20)
        # Assemble: stock_row (10) + rel-strength (2) at indices 10-11, then NASDAQ block.
        feature_rows[i, :10] = stock_row
        feature_rows[i, 10] = rel_strength_1d
        feature_rows[i, 11] = rel_strength_20d
        feature_rows[i, ML_BRAIN_STOCK_FEATURES:] = nasdaq_row
...
def _ml_hist_fetch_symbol_universe():
    """Uses the SAME in-memory candidate list the bot itself trades from,
    so the model trains on symbols the bot will actually see. Reads the
    module-level SYMBOLS_TO_BUY_LIST populated by the inlined scanner --
    no text files."""
    try:
        return get_symbols_to_buy()
    except Exception:
        return []
...
def _ml_train_on_examples(examples, run_kind):
    ...
    # CHRONOLOGICAL train/val split (V2 improvement -- was: random shuffle).
    # Random splitting was leaking future information: because examples span
    # many correlated symbols on the same calendar days, a random 15% val set
    # ends up containing dates that overlap heavily with the training set,
    # and the reported val_accuracy is optimistically biased. Sorting by
    # anchor date and taking the LAST 15% as val gives us a proper
    # out-of-time validation slice that mirrors what live inference actually
    # faces: predicting today's setups from a model trained on strictly
    # earlier data. Undated examples (mocks, or dfs without DatetimeIndex)
    # sink to the end via the same sentinel used in walk-forward eval.
    def _final_sort_key(i):
        e = examples[i]
        d = e[2] if len(e) >= 3 else None
        return d if d is not None else datetime(9999, 1, 1)

    chrono_order = sorted(range(len(examples)), key=_final_sort_key)
    chrono_idx = np.array(chrono_order, dtype=np.int64)
    X, y, sw = X[chrono_idx], y[chrono_idx], sample_weight[chrono_idx]
    X_train, X_val = X[:-n_val], X[-n_val:]
    y_train, y_val = y[:-n_val], y[-n_val:]
    sw_train = sw[:-n_val]

    # Shuffle only the TRAINING slice (keeps val strictly future-of-train).
    rng = np.random.default_rng(42)
    train_perm = rng.permutation(len(X_train))
    X_train, y_train, sw_train = X_train[train_perm], y_train[train_perm], sw_train[train_perm]

    with _ml_lock:
        model = _ml_load_cached_model(tf) or _ml_build_model(tf)
        epochs = 6 if run_kind == 'first_run' else 20  # was 12; EarlyStopping caps effective epochs
        # EarlyStopping (V2 improvement): halt when val_loss stops improving,
        # then restore the weights from the best epoch. Prevents the LSTM head
        # from grinding into overfit territory on the 15k daily run without
        # forcing us to hand-tune the epoch count. patience=3 gives the
        # optimizer some slack for noisy val_loss between epochs but pulls the
        # plug fast on genuine divergence. min_delta requires a real
        # improvement, not just floating-point wiggle.
        early_stop = keras.callbacks.EarlyStopping(
            monitor='val_loss', patience=3, min_delta=1e-4,
        ...
        # DATA-DRIVEN INFERENCE THRESHOLD (V2 improvement).
        # Focal loss + per-sample loss penalty produce probabilities that are
        # NOT calibrated around 0.5 -- the model can be a strong classifier
        # while its raw output is biased high or low. Rather than hard-coding
        # 0.5 as the "no adjustment" point (which was silently biasing the
        # score adjustment sign), compute the median predicted probability on
        # the held-out val set and use THAT as the neutral point. Positive
        # score adjustment only when prob > median; negative when < median.
        # Saved to meta.json so live inference can load it and drop back to
        # 0.5 only when the file is missing/unreadable.
        try:
            val_probs = model.predict(X_val, verbose=0).flatten()
            inference_neutral = float(np.median(val_probs))
            # Guardrail: if the model outputs are pathologically concentrated
            # (e.g. every prediction ~= 0.5 because training barely moved
            # the head), fall back to 0.5. Otherwise a nearly-flat model
            # could produce huge adjustments on trivial prob differences.
            val_prob_spread = float(val_probs.std())
            if val_prob_spread < 0.02:
                inference_neutral = 0.5
        except Exception as e:
            logging.warning(f"ml_brain: val threshold calibration failed ({e}); using 0.5")
            inference_neutral = 0.5
            val_prob_spread = 0.0
        ...
        meta = {
            ...
            # V2 additions:
            'inference_neutral_prob': inference_neutral,   # replaces hard-coded 0.5
            'val_prob_spread': val_prob_spread,             # for diagnostics
            'epochs_run': len(history.history.get('loss', [])),  # early-stopped or full
            'feature_count': ML_BRAIN_FEATURES,             # for version guard
        }
        ...
        _ml_model_cache['inference_neutral_prob'] = inference_neutral  # V2
...
def _ml_load_cached_model(tf):
    ...
    with _ml_lock:
        ...
        try:
            ...
            # V2 shape guard: an old V1 model (10-feature input) on disk would
            # load fine here but crash on the first inference call with a shape
            # mismatch. Detect it and force a rebuild instead of returning a
            # model the caller can't actually use.
            try:
                input_features = int(model.input_shape[-1])
            except Exception:
                input_features = None
            if input_features != ML_BRAIN_FEATURES:
                logging.warning(
                    f"ml_brain: saved model has {input_features}-feature input "
                    f"but current code expects {ML_BRAIN_FEATURES} (V2). "
                    f"Ignoring saved model; next training run will overwrite it.")
                return None
            model.compile(optimizer=keras.optimizers.Adam(learning_rate=ML_BRAIN_LEARNING_RATE),
                          loss=_ml_make_focal_loss(ML_BRAIN_FOCAL_GAMMA),
                          metrics=['accuracy'])
            _ml_model_cache['model'] = model
            _ml_model_cache['inference_neutral_prob'] = None  # V2: force re-read from meta
            if os.path.exists(ML_META_PATH):
                ...
                _ml_model_cache['inference_neutral_prob'] = meta.get('inference_neutral_prob')
            return model
        except Exception as e:
...
def _ml_current_feature_row_for_symbol(symbol, df, rsi_val, macd_val,
    ...
    # V2: get the NASDAQ market-state map. Cached, so this is cheap on hot path.
    nasdaq_map = _ml_get_nasdaq_features_map()
    has_datetime_index = hasattr(df.index, 'to_pydatetime')
    ...
    for j, i in enumerate(range(start, start + n_needed)):
        ...
        if has_datetime_index:
            ...
            bar_date = None
        nasdaq_row = _ml_lookup_nasdaq_row_for_date(nasdaq_map, bar_date)
        # V2 relative-strength (must match training-time formula exactly)
        nas_daily = nasdaq_row[5] if len(nasdaq_row) > 5 else 0.0
        nas_ret20 = nasdaq_row[7] if len(nasdaq_row) > 7 else 0.0
        if i >= 20 and close[i-20] > 0:
            stock_ret20 = (close[i] - close[i-20]) / close[i-20]
        else:
            stock_ret20 = 0.0
        ...
        rel_strength_20d = float(stock_ret20) - float(nas_ret20)
        seq[j, :10] = stock_row
        seq[j, 10] = rel_strength_1d
        seq[j, 11] = rel_strength_20d
        seq[j, ML_BRAIN_STOCK_FEATURES:] = nasdaq_row
    return seq
...
def get_ml_adjustment(sess, tf_model, buy_score=None, rsi=None, atr_pct=None,
    ...
    # V2: use the data-driven neutral point saved during training, not a
    # hard-coded 0.5. Focal loss + class weighting typically produce
    # probabilities that are miscalibrated around 0.5, and using 0.5 as the
    # break-even point was silently biasing every adjustment in the direction
    # of whichever side the model happened to be shifted toward. Cached on
    # the module cache so we don't re-read meta.json on every inference call.
    neutral = _ml_model_cache.get('inference_neutral_prob')
    if neutral is None:
        try:
            if os.path.exists(ML_META_PATH):
                with open(ML_META_PATH) as f:
                    meta = _ml_json.load(f)
                neutral = float(meta.get('inference_neutral_prob', 0.5))
            else:
                neutral = 0.5
        except Exception:
            neutral = 0.5
        _ml_model_cache['inference_neutral_prob'] = neutral

    # Adjustment scales linearly from -MAX at prob=0 to +MAX at prob=1, passing
    # through zero at the calibrated neutral point. Clipped to the cap
    # regardless of neutral placement.
    if prob >= neutral:
        # Positive side: prob-neutral in [0, 1-neutral] -> [0, +MAX]
        span = max(1e-6, 1.0 - neutral)
        adjustment = ((prob - neutral) / span) * ML_MAX_SCORE_ADJUSTMENT
    else:
        # Negative side: prob-neutral in [-neutral, 0] -> [-MAX, 0]
        span = max(1e-6, neutral)
        adjustment = ((prob - neutral) / span) * ML_MAX_SCORE_ADJUSTMENT
    adjustment = max(-ML_MAX_SCORE_ADJUSTMENT,
                     min(ML_MAX_SCORE_ADJUSTMENT, adjustment))
...
def _backtest_print_summary(report):
    ...
    print(f"Net P&L:            ${report['net_pnl']:,.2f}  ({report['net_pnl_pct']:+.2f}%)")
    print(f"Total trades:       {report['total_trades']}  "
          f"({report['wins']}W / {report['losses']}L, "
          f"win rate {report['win_rate_pct']:.1f}%)")
    print(f"Avg winner P&L:     ${report['avg_win_pnl']:,.2f}")
    print(f"Avg loser P&L:      ${report['avg_loss_pnl']:,.2f}")
...
def report_expectancy():
    ...
    print(f"  Median winner:       {med_win*100:+.2f}%    Median loser: {med_loss*100:+.2f}%")
    print(f"  Expectancy / trade:  {expectancy*100:+.3f}%")
...
# Updated to the 2026 constituent list from S&P Dow Jones Indices / Wikipedia
# (503 tickers -- the index carries 500 companies but 3 have dual share classes:
# Alphabet GOOG/GOOGL, Fox Corp FOX/FOXA, News Corp NWS/NWSA).
#
# Recent constituent changes captured here that were missing from the prior
# list, plus recent ticker-symbol changes that yfinance now expects:
#   MRSH: Marsh McLennan changed NYSE ticker from MMC to MRSH on 2026-01-14
#         (rebranded to "Marsh"). yfinance now only serves data under MRSH,
#         so MMC is NOT included as a fallback -- it just returned empty
#         frames and wasted a scanner slot.
#   FISV: Fiserv changed from FI to FISV on 2025-11-11 when it moved from
#         NYSE to Nasdaq. yfinance fetches under FISV going forward.
#   ECHO: EchoStar changed Nasdaq ticker from SATS to ECHO on 2026-06-24.
#         Wikipedia's S&P 500 table still shows SATS as of this file's writing;
#         yfinance only serves ECHO now, so ECHO is what's in the list.
#   BRK-B / BF-B: yfinance uses dashes, not the dots that S&P/Wikipedia show
#         (BRK.B / BF.B) -- kept as dashes here to match the yfinance API.
#
# Ticker share-class variants (BRK.B, BF.B) are normalized to dashes here to
# match the yfinance API convention that _scanner_batch_download expects.
# If a symbol fails to fetch (delisted, temporary yfinance gap, new IPO not
# yet in the yfinance backfill), _scanner_batch_download will retry and the
# scanner simply skips it -- one bad symbol never blocks the rest of the run.
_SCANNER_SP500_UNIVERSE = [
    ...
    'BDX', 'BRK-B', 'BBY', 'TECH', 'BIIB', 'BLK', 'BX', 'XYZ', 'BNY', 'BA', 'BKNG', 'BSX', 'BMY', 'AVGO', 'BR',
    ...
    'CMCSA', 'FIX', 'CAG', 'COP', 'ED', 'STZ', 'CEG', 'COO', 'CPRT', 'GLW', 'CPAY', 'CTVA', 'CSGP', 'COST', 'CRH',
    'CRWD', 'CCI', 'CSX', 'CMI', 'CVS', 'DHR', 'DRI', 'DDOG', 'DVA', 'DECK', 'DE', 'DELL', 'DAL', 'DVN', 'DXCM',
    'FANG', 'DLR', 'DG', 'DLTR', 'D', 'DPZ', 'DASH', 'DOV', 'DOW', 'DHI', 'DTE', 'DUK', 'DD', 'ETN', 'EBAY',
    'ECHO', 'ECL', 'EIX', 'EW', 'EA', 'ELV', 'EME', 'EMR', 'ETR', 'EOG', 'EPAM', 'EQT', 'EFX', 'EQIX', 'EQR',
    'ERIE', 'ESS', 'EL', 'EG', 'EVRG', 'ES', 'EXC', 'EXE', 'EXPE', 'EXPD', 'EXR', 'XOM', 'FFIV', 'FDS', 'FICO',
    'FAST', 'FRT', 'FDX', 'FIS', 'FITB', 'FSLR', 'FE', 'FISV', 'F', 'FTNT', 'FTV', 'FOXA', 'FOX', 'BEN', 'FCX',
    'GRMN', 'IT', 'GE', 'GEHC', 'GEV', 'GEN', 'GNRC', 'GD', 'GIS', 'GM', 'GPC', 'GILD', 'GPN', 'GL', 'GDDY',
    'GS', 'HAL', 'HIG', 'HAS', 'HCA', 'DOC', 'HSIC', 'HSY', 'HPE', 'HLT', 'HD', 'HON', 'HRL', 'HST', 'HWM',
    'HPQ', 'HUBB', 'HUM', 'HBAN', 'HII', 'IBM', 'IEX', 'IDXX', 'ITW', 'INCY', 'IR', 'PODD', 'INTC', 'IBKR', 'ICE',
    'IFF', 'IP', 'INTU', 'ISRG', 'IVZ', 'INVH', 'IQV', 'IRM', 'JBHT', 'JBL', 'JKHY', 'J', 'JNJ', 'JCI', 'JPM',
    'KVUE', 'KDP', 'KEY', 'KEYS', 'KMB', 'KIM', 'KMI', 'KKR', 'KLAC', 'KHC', 'KR', 'LHX', 'LH', 'LRCX', 'LVS',
    'LDOS', 'LEN', 'LII', 'LLY', 'LIN', 'LYV', 'LMT', 'L', 'LOW', 'LULU', 'LITE', 'LYB', 'MTB', 'MPC', 'MAR',
    'MRSH', 'MLM', 'MAS', 'MA', 'MKC', 'MCD', 'MCK', 'MDT', 'MRK', 'META', 'MET', 'MTD', 'MGM', 'MCHP', 'MU',
    'MSFT', 'MAA', 'MRNA', 'TAP', 'MDLZ', 'MPWR', 'MNST', 'MCO', 'MS', 'MOS', 'MSI', 'MSCI', 'NDAQ', 'NTAP',
    ...
    'VLO', 'VEEV', 'VTR', 'VLTO', 'VRSN', 'VRSK', 'VZ', 'VRTX', 'VRT', 'VTRS', 'VICI', 'V', 'VST', 'VMC', 'WRB',
    'GWW', 'WAB', 'WMT', 'DIS', 'WBD', 'WM', 'WAT', 'WEC', 'WFC', 'WELL', 'WST', 'WDC', 'WY', 'WSM', 'WMB',
...
def _scanner_batch_download(symbols, start_date, end_date, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            for s in symbols:
                sd = data[s] if s in data else pd.DataFrame()
                if not sd.empty and sd['Close'].dropna().size > 0:
                    valid.append(s)
                else:
                    invalid.append(s)
            time.sleep(1)
            ...
            logging.error(f"scanner: batch download attempt {attempt+1} failed: {e}")
            if attempt < retries - 1:
                time.sleep(5 * (2 ** attempt))

    # Fallback: smaller batches
    all_data, valid, invalid = {}, [], []
    bsize = _SCANNER_CONFIG['batch_size_fallback']
    for i in range(0, len(symbols), bsize):
        batch = symbols[i:i + bsize]
        try:
            bd = yf.download(tickers=batch, start=start_date, end=end_date,
                             group_by='ticker', threads=True, progress=False)
            for s in batch:
                sd = bd[s] if s in bd else pd.DataFrame()
                if not sd.empty and sd['Close'].dropna().size > 0:
                    all_data[s] = sd
                    valid.append(s)
                else:
            ...
            logging.error(f"scanner: fallback batch {i} failed: {e}")
    if all_data:
        return pd.concat(all_data, axis=1, keys=all_data.keys()), valid, invalid
