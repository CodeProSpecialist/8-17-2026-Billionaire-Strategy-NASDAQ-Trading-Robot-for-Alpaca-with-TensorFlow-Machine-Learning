...
def _bull_monitor_symbol_ticks(symbol, duration, interval, results, results_lock):
    """
    Worker for _bull_market_scan_candidates. Samples get_current_price every
    `interval` seconds for `duration` seconds, counting up-ticks and down-ticks
    vs. the previous sample. Writes {'increased': N, 'decreased': N, 'samples': N,
    'last_price': X} into results[symbol] under results_lock.

    Uses the bot's cached get_current_price (2-minute TTL) so many parallel
    workers do not overload the yfinance rate limiter -- most samples are
    served from cache and only refresh every ~2 minutes.
    """
    increased = 0
    decreased = 0
    samples = 0
    ...
    while time.time() < stop_at:
        try:
            ...
            logging.warning(f"[bull] tick monitor error for {symbol}: {e}")
        # Sleep the interval, but wake early if we're about to overshoot.
...
def _bull_market_scan_candidates(symbols_to_buy_list, regime, total_symbols_for_alloc):
    ...
    print(f"[bull] monitoring {len(symbols_to_buy_list)} symbol(s) for "
          f"{BULL_MONITOR_SECONDS}s (sample every {BULL_SAMPLE_INTERVAL}s)...")
    tick_results = {}
    tick_lock = threading.Lock()
    threads = []
    for symbol in list(symbols_to_buy_list):
        t = threading.Thread(
            target=_bull_monitor_symbol_ticks,
            args=(symbol, BULL_MONITOR_SECONDS, BULL_SAMPLE_INTERVAL,
                  tick_results, tick_lock),
            daemon=True,
        )
        threads.append(t)
        t.start()
    for t in threads:
        # Cap join at duration + a small safety margin; a stuck worker won't
        # hold the whole buy cycle hostage.
        t.join(timeout=BULL_MONITOR_SECONDS + 30)
    ...
    for symbol in list(symbols_to_buy_list):
        ...
        try:
            ...
            if ticks is None or ticks['samples'] < 2:
                print(f"[bull] {yf_symbol}: no tick data (samples={ticks['samples'] if ticks else 0}). Skipping.")
                continue
