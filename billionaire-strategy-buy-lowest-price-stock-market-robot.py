...
def _bootstrap_dependencies():
    ...
    # Install. Try normal install first; fall back to --break-system-packages
    # for externally-managed Debian/Ubuntu Python 3.12+ environments.
    for pkg in missing:
        installed_ok = False
        for args in (
            [_boot_sys.executable, "-m", "pip", "install", "--quiet", pkg],
            [_boot_sys.executable, "-m", "pip", "install", "--quiet",
             "--break-system-packages", pkg],
        ):
            try:
                r = _boot_subprocess.run(args, capture_output=True,
